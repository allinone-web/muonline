...
{
    /// <summary>
    /// Devil Square entry window (Charon). Matches SourceMain5.2 logic and modern UI theme.
    /// </summary>
    public sealed class DevilSquareEnterControl : UIControl
    {
        private const int ENTRY_COUNT = 7;
        private const int ENTRY_ROW_HEIGHT = 34;
        private const int ENTRY_ROW_GAP = 6;
        ...
        private static DevilSquareEnterControl _instance;
        ...
        private bool _hasMatchingInvitation;
        private bool _hasAnyInvitation;
        private byte _matchingInvitationSlot = 0xFF;
        private byte _expectedInvitationLevel;
        private byte _foundInvitationLevel;
        ...
        private DevilSquareEnterControl()
        {
        ...
        private void DrawCloseButton(SpriteBatch sb)
        {
            if (MobileUi.IsMobile)
            {
                ...
                // 粗細、顏色、有沒有底都不一樣。
                MobileUi.DrawCloseGlyph(sb, Translate(_closeButtonRect), _closeHovered);
                return;
            }
        ...
        private void DrawEntryRows(SpriteBatch sb)
        {
            for (int i = 0; i < ENTRY_COUNT; i++)
            {
                ...
                if (isActive && !_hasMatchingInvitation)
                {
                    rightColor = _hasAnyInvitation ? Theme.Warning : Theme.Danger;
                }
                ...
                if (isActive)
                {
                    string actionText = _hasMatchingInvitation ? "ENTER" : "INVITATION";
                    Color actionColor = _hasMatchingInvitation ? Theme.Success : Theme.Warning;
        ...
        {
            string gateText = $"Active Gate: Devil Square {_activeIndex + 1}";
            string statusText;
            Color statusColor;

            if (_hasMatchingInvitation)
            {
                statusText = $"Invitation OK: Devil's Invitation +{_expectedInvitationLevel}";
                statusColor = Theme.Success;
            }
            else if (_hasAnyInvitation)
            {
                statusText = $"Invitation mismatch: need +{_expectedInvitationLevel}, have +{_foundInvitationLevel}";
                statusColor = Theme.Warning;
            }
            else
            {
                statusText = "Devil's Invitation required to enter.";
        ...
        private void HandleEntryClick(int index)
        {
            ...
            if (!_hasMatchingInvitation)
            {
                string msg = _hasAnyInvitation
                    ? $"Devil's Invitation +{_expectedInvitationLevel} required."
                    : "Devil's Invitation required to enter.";
            ...
            if (svc != null)
            {
                _ = svc.SendDevilSquareEnterRequestAsync((byte)_activeIndex, _matchingInvitationSlot);
            }
        ...
        {
            _hasMatchingInvitation = false;
            _hasAnyInvitation = false;
            _matchingInvitationSlot = 0xFF;
            _foundInvitationLevel = 0;
            ...
            foreach (var item in _characterState.GetInventoryItems())
            {
                ...
                _hasAnyInvitation = true;
                var details = ItemDatabase.ParseItemDetails(data);
                _foundInvitationLevel = (byte)details.Level;

                if (details.Level == _expectedInvitationLevel)
                {
                    _hasMatchingInvitation = true;
                    _matchingInvitationSlot = item.Key;
                    break;
                }
            }
        }
