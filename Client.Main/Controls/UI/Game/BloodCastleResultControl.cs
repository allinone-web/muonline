    ...
    public sealed class BloodCastleResultControl : UIControl
    {
        ...
        private void DrawCloseButton(SpriteBatch sb)
        {
            if (MobileUi.IsMobile)
            {
                ...
                // 粗細、顏色、有沒有底都不一樣。
                MobileUi.DrawCloseGlyph(sb, new Rectangle(DisplayRectangle.X + _closeButtonRect.X, DisplayRectangle.Y + _closeButtonRect.Y, _closeButtonRect.Width, _closeButtonRect.Height), _closeHovered);
                return;
            }
